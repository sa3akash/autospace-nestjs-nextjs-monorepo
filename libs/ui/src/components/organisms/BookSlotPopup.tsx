...
export const BookSlotPopup = ({
  ...
  const handleBookingStripe = async (data: FormTypeBookSlot) => {
    if (!id) {
    ...
    try {
      ...
      const res = await createBookingSession(id!, totalPriceObj, bookingData);
      console.log(res);
    } catch {
  ...
  return (
    <div className="flex gap-2 text-left border-t-2 border-white bg-white/50 backdrop-blur-sm">
      <Form onSubmit={handleSubmit(handleBookingStripe)}>
